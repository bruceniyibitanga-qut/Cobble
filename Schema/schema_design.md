...
### 3.5 Soft deletes
Every business table has `deleted_at` (NULL = live record). In Postgres this was a partial index (`WHERE deleted_at IS NULL`); MySQL doesn't support partial indexes, so we index the columns unconditionally and rely on application-level filtering. Slightly less efficient on huge tables, fine at our scale. Nothing gets permanently deleted — records stay in the DB and can be restored or audited.

### 3.6 Audit log
Changes are written to `audit_log` with old + new values as JSON. This survives soft deletes, is queryable by record or by user, and gives you a full history of what changed. Recommend writing from the API layer rather than DB triggers so you can capture the HTTP user's ID and IP address.

### 3.7 UUID primary keys
UUIDs (stored as `CHAR(36)`) avoid collisions when importing data from multiple Excel spreadsheets, and don't leak record counts in URLs. Generated by the application (C# `Guid.NewGuid()`) rather than the database. Small storage overhead at this scale, not worth worrying about.
...
- FULLTEXT indexes on `organisations.name` and `projects.title` for fuzzy search (CO-2)
- The "one primary contact per org" rule is now enforced at the application layer (MySQL doesn't support the partial unique index Postgres used)
